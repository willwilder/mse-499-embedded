...
#Temperature controller console application
#(C) 2015 Will Wilder <will.wilder@gmail.com>
#
# This project based on miniterm
#
...
import sys, os, serial, threading, time, PID, csv
from struct import *
try:
...
EXITCHARCTER = serial.to_bytes([0x03])   # GS/CTRL+C
...
CSV_HEADER = "ProcessRunning,SP,PV,Kp,Ki,Kd,Error,PError,Integral,Derivative,Output"
...
class Miniterm(object):
    ...
    def _start_recipe(self):
        ...
        self.recipe_thread = threading.Thread(target=self.run_recipe)
        print "starting recipe"
        self.recipe_thread.setDaemon(True)
        self.recipe_thread.start()

    def _stop_recipe(self):
        """Stop recipe thread only, wait for clean exit of thread"""
        self._recipe_alive = False
        self.recipe_thread.join()
    ...
    def downloadPIDParameters(self):
        ...
        print(PID.Ki)
        print(PID.Kd)
        #self.serial.write(pack('ccc','P',chr(PID.Kp),'\n'))
        self.serial.write('P')
        time.sleep(.1)
        self.serial.write(chr(PID.Kp))
        time.sleep(.1)
        self.serial.write('\n')
        time.sleep(.1)
        self.serial.write('I')
        time.sleep(.1)
        self.serial.write(chr(PID.Ki))
        time.sleep(.1)
        self.serial.write('\n')
        time.sleep(.1)
        self.serial.write('D')
        time.sleep(.1)
        self.serial.write(chr(PID.Kd))
        time.sleep(.1)
        self.serial.write('\n')
        time.sleep(.1)
    ...
    def run_recipe(self):
        ...
        self.run_process()
        print "currentSP: " + str(PID.currentSP)
        ...
        try:
            while self.alive and self._reader_alive:
                ...
                if (not self.previous_process_runnning and self.process_running):
                    ...
                    self.csv_writer(CSV_HEADER.split(","),self.current_csv_path)
            ...
            while self.alive:
                ...
                if menu_active:
                    if c == MENUCHARACTER or c == EXITCHARCTER: # Menu character again/exit char -> send itself
                        ...
                        if self.echo:
                            sys.stdout.write(c)
                    elif c == '\x15':                       # CTRL+U -> upload recipe
                        print "recipe alive: " + str(self._recipe_alive)
                        self._start_recipe()
                        #if self._recipe_alive:
                         #   self._stop_recipe()
                          #  self.stop_process
                        #else:
                         #   print "starting recipe fadlsdf"
                          #  self._start_recipe()
                        #self.printit()
                        #self.update_setpoint(100)
                    elif c == '\x10':                       # CTRL+P -> Run Process
